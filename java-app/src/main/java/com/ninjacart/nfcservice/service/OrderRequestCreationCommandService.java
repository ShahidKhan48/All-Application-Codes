...
@Service
@Slf4j
public class OrderRequestCreationCommandService extends AbstractCommandCreationService {
...
//    commandCreationObjectDto.setCreatedBy(initiatedByDto.getNfcUserId());
  ...
  private OrderManagementRequestObjectDto checkWhetherRequestIsAllowedOrNot(
    ...
    if (!CollectionUtils.isEmpty(approvalsDtos) && orderManagementRequestObjectDto.getEntityStatus().contains("NEGOTIATION")) {
      for (ApprovalsDto each : approvalsDtos) {
        if (StringUtils.isEmpty(each.getStatus())
          ...
          throw CommonUtils.logAndGetException("Order doesn't have required approvals");
        }
      }
    }
