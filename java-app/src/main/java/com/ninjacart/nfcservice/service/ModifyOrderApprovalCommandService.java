...
@Slf4j
@Service
public class ModifyOrderApprovalCommandService extends AbstractCommandProcessorService {
  ...
  private static final String ORDER_APPROVED_STATUS = "ORDER_MODIFY_APPROVED";
  private static final String ORDER_REJECTED_STATUS = "ORDER_MODIFY_REJECTED";
  ...
  public CommandProcessorResponseDto processV1(
    ...
    CommandStatusUpdateDto commandStatusUpdateDto =
        parseAndCheckMandatoryValues(commandStatusUpdateWrapperDto);
    ...
    RequestCreationResponse requestCreationResponse =
        updateDataInOms(
            ...
            commandStatusUpdateDto.getInitiatedByDto(),
    ...
    if (!StringUtils.isEmpty(requestCreationResponse.getEntityStatus())
        && requestCreationResponse.getEntityStatus().equalsIgnoreCase(ORDER_APPROVED_STATUS)) {
      triggerOrderCreatedMessage(
          statusUpdateDtoRequest.getExternalReferenceId(), statusUpdateDtoRequest.getRoomId(), requestCreationResponse);
    }
  ...
  private MessageResponse getAndUpdateTheMessageWithStatus(
      String eventId, String roomId, String token, RequestCreationResponse requestCreationResponse)
      throws Exception {
  ...
  private RequestCreationResponse updateDataInOms(
      CommandStatusRequestObjectDto statusUpdateDtoRequest,
      InitiatedByDto initiatedByDto,
      OrderManagementRequestObjectDto orderManagementRequestObjectDto) {
    List<ApprovalsDto> approvalsDtos = orderManagementRequestObjectDto.getApprovals();
    boolean approved =
        statusUpdateDtoRequest.getStatus().equals(OrderRequestCommandStatusEnum.APPROVED.name());

    approvalsDtos = updateApprovals(approvalsDtos, approved, initiatedByDto.getNfcUserId());
    orderManagementRequestObjectDto.setApprovals(approvalsDtos);

    orderManagementRequestObjectDto =
        checkApprovalStatusAndUpdateData(orderManagementRequestObjectDto, statusUpdateDtoRequest);
    orderManagementRequestObjectDto.setEntityVersion(
  ...
  private OrderManagementRequestObjectDto checkApprovalStatusAndUpdateData(
      OrderManagementRequestObjectDto orderManagementRequestObjectDto,
      CommandStatusRequestObjectDto statusUpdateDtoRequest) {
    ...
    if (!CollectionUtils.isEmpty(orderManagementRequestObjectDto.getApprovals())) {
      for (ApprovalsDto each : orderManagementRequestObjectDto.getApprovals()) {
        if (StringUtils.isEmpty(each.getStatus())
            || !each.getStatus().equalsIgnoreCase(OrderRequestCommandStatusEnum.APPROVED.name())) {
          approved = false;
          break;
        }
      }
  ...
  private CommandStatusUpdateDto parseAndCheckMandatoryValues(
    ...
    if (requestObjectDto == null
        || requestObjectDto.getEntityVersion() == null
        || StringUtils.isEmpty(requestObjectDto.getCommandEventId())
        || StringUtils.isEmpty(requestObjectDto.getRoomId())
        || StringUtils.isEmpty(requestObjectDto.getExternalReferenceId())) {
      throw CommonUtils.logAndGetException("Invalid Input");
    }
  ...
  private void checkApprovalStatusAndCreateOrder(
      CommandStatusUpdateWrapperDto commandStatusUpdateWrapperDto) throws Exception {
    ...
    //    }
    //
    //    List<ConsentsFetchResponseDto> consentsFetchResponseDtos =
    // workflowRestService.fetchConsents(OrderManagementFetchFilter.builder()
    //
    ...
    //        ObjectMapper mapper = new ObjectMapper();
    //        triggerTenantApis(mapper.readValue(each.getConsentData(), RequestObjectDto.class),
    // Context.builder()
  ...
  private void triggerOrderCreatedMessage(String externalReferenceId, String roomId, RequestCreationResponse requestCreationResponse) {
    ...
    CommandMessageBodyDto commandMessageBodyDto =
        CommandMessageBodyDto.builder()
            .command(CommandTypeEnum.MODIFY_ORDER_CREATED.name())
            .data(
  ...
  private void triggerTenantApis(RequestObjectDto requestObjectDto, Context context) {
    ...
    forwardRequestToTenant(
        RequestDto.builder()
            .context(context)
            .message(Message.builder().request(requestObjectDto).build())
            .build(),
        SELLER_ORDER_CREATION_TENANT_URL);
    ...
    forwardRequestToTenant(
        RequestDto.builder()
            ...
            .message(Message.builder().request(requestObjectDto).build())
            .build(),
        BUYER_ORDER_CREATION_TENANT_URL);
  }
  ...
  private void editApprovalMessage(CommandStatusUpdateWrapperDto commandStatusUpdateWrapperDto)
    ...
    //        mapper.convertValue(commandMessageBodyDto.getData(),
    // OrderRequestCommandApprovalDto.class);
    //
    //    orderRequestCommandApprovalDto = updateApprovalStatus(orderRequestCommandApprovalDto,
  ...
  private void triggerOrderRequestStatus(
      CommandStatusUpdateWrapperDto commandStatusUpdateWrapperDto) {}
  ...
  private void sendAudit( String roomId, InitiatedByDto initiatedByDto,CommandStatusRequestObjectDto commandStatusRequestObjectDto, RequestCreationResponse requestCreationResponse)  {
    ...
    CommandMessageBodyDto commandMessageBodyDto =
        CommandMessageBodyDto.builder()
            .command(CommandTypeEnum.ORDER_MODIFY_APPROVAL_AUDIT.name())
